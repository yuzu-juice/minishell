...
/*   Updated: 2025/03/23 16:36:40 by takitaga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	forth_env = third_env->next;
	assert(forth_env != NULL);
	assert(strcmp(forth_env->key, "SPECIAL") == 0);
	assert(strcmp(forth_env->value, "value with=spaces:another$value:third*value") == 0);
	assert(forth_env->next == NULL);
